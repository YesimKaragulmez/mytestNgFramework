...
public class Priority_DependsOnMethod {
    ...
    public void search() {
        ...
        Assert.assertEquals(3, 5); //this method fails,result method  will be ignored
        //"test ignored" message will be

    }

    //To Make result() depends on search(), we
    @Test(priority = 4, dependsOnMethods = "search")
//result() is now dependent on result.search() is not dependent on result()
    public void result() {
